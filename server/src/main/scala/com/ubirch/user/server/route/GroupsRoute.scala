...
/**
  ...
  * since: 2017-03-30
  */
class GroupsRoute(implicit mongo: MongoUtil) extends MyJsonProtocol
  with CORSDirective
  with ResponseUtil
  with StrictLogging {
  ...
  private val groupsActor = system.actorOf(new RoundRobinPool(Config.akkaNumberOfWorkers).props(Props(new GroupsActor)), ActorNames.GROUPS)
